	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="采购总价" prop="caigouzongjia">
				<el-input v-model="caigouzongjia" placeholder="采购总价" disabled></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="采购备注" prop="caigoubeizhu">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.caigoubeizhu"
					placeholder="采购备注"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`yuanliaocaigou/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				var obj = JSON.parse(localStorage.getItem('crossObj'));
				if(obj.kucun<this.ruleForm.kucun){
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						if(this.type=='cross'){
							...
							var obj = JSON.parse(localStorage.getItem('crossObj'));
							var table = localStorage.getItem('crossTable');
							obj.kucun = Number((parseFloat(obj.kucun) - parseFloat(this.ruleForm.kucun)).toFixed(2))
							await this.$http.post(table+`/update`,obj).then(res => {});
						}

						await this.$http.post(`yuanliaocaigou/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
