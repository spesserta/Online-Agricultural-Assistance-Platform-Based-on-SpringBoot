	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="评价状态" prop="pingjiazhuangtai">
				<el-select v-model="ruleForm.pingjiazhuangtai" placeholder="请选择评价状态" :disabled="ro.pingjiazhuangtai"  filterable>
					<el-option
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`caigoupingjia/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`caigoupingjia/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
