...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="使用说明" prop="shiyongshuoming">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.shiyongshuoming"
					placeholder="使用说明"
					v-model="ruleForm.shiyongshuoming">
					</el-input>
			</el-form-item>
			<el-form-item class="add-item" label="原料介绍" prop="yuanliaojieshao">
				<editor 
					v-model="ruleForm.yuanliaojieshao" 
					class="editor" 
					myQuillEditor="yuanliaojieshao"
					action="file/upload">
				</editor>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='yuanliaojieshao'){
							this.ruleForm.yuanliaojieshao = obj[o];
							this.ro.yuanliaojieshao = true;
							continue;
						}
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`nongyeyuanliao/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`nongyeyuanliao/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
