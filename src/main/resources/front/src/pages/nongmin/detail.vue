	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'-'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/nongmin?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
				...
				<div class="info">
					<div class="title-item">
						<div class="detail-title">
							{{detail.nongminzhanghao}}
						</div>
					</div>
					<div class="item">
						<div class="lable">农民姓名</div>
						<div class="text "  >{{detail.nongminxingming}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('nongmin','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('nongmin','删除')" @click="delClick">删除</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('nongmin','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(async res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.nongminzhanghao;
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/nongmin', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/nongminAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此农民？') .then(_ => {
					this.$http.post('nongmin/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
