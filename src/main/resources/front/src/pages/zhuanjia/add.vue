...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="从业年限" prop="congyenianxian">
				<el-input v-model="ruleForm.congyenianxian" 
					placeholder="从业年限" clearable :readonly="ro.congyenianxian"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="研究领域" prop="yanjiulingyu">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.yanjiulingyu"
					placeholder="研究领域"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`zhuanjia/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`zhuanjia/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
