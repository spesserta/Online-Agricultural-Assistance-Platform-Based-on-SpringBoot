	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'-'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/zhuanjia?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">从业年限</div>
						<div class="text "  >{{detail.congyenianxian}}</div>
					</div>
					<div class="item">
						<div class="lable">研究领域</div>
						<div class="text "  >{{detail.yanjiulingyu}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('zhuanjia','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('zhuanjia','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('zhuanjia','专家预约')" 
							@click="onAcross('zhuanjiayuyue','','','','')" type="warning">
							专家预约
						</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('zhuanjia','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`zhuanjia`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/zhuanjia', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此专家？') .then(_ => {
