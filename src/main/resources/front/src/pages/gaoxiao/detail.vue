	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'-'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/gaoxiao?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
				...
				<div class="info">
					<div class="title-item">
						<div class="detail-title">
							{{detail.gaoxiaomingcheng}}
						</div>
					</div>
					<div class="item">
						<div class="lable">高校地址</div>
						<div class="text "  >{{detail.gaoxiaodizhi}}</div>
					</div>
					<div class="item">
						<div class="lable">高校电话</div>
						<div class="text "  >{{detail.gaoxiaodianhua}}</div>
					</div>
					<div class="item">
						<div class="lable">高校简介</div>
						<div class="text "  >{{detail.gaoxiaojianjie}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('gaoxiao','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('gaoxiao','删除')" @click="delClick">删除</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('gaoxiao','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(async res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.gaoxiaomingcheng;
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/gaoxiao', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/gaoxiaoAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此高校？') .then(_ => {
					this.$http.post('gaoxiao/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chaptergaoxiao', query: params});
			},
			// 获取uuid
			getUUID() {
