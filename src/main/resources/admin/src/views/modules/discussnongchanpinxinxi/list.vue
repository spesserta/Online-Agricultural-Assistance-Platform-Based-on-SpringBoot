	...
	<div class="main-content" :style='{"minHeight":"100vh","padding":"10px 20px","background":" linear-gradient( 180deg, #EFFFEF 0%, #F8F8F8 100%)"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 30px 0","borderRadius":"16px","background":" linear-gradient( 180deg, #D5F5D3 0%, #FFFFFF 100%)"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"width":"100%","padding":"30px","position":"relative","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 12px 0 10px","outline":"none","color":"#fff","borderRadius":"24px","background":"#29C41F","width":"auto","fontSize":"16px","minWidth":"110px","fontWeight":"bold","height":"41px"}' type="success" @click="backClick()">返回</el-button>
				</el-row>

				<el-row class="actions" :style='{"width":"100%","padding":"0 30px 30px 30px","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('discussnongchanpinxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('discussnongchanpinxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"20px","borderColor":"#212D3F","borderRadius":"16px","background":"linear-gradient( 180deg, #D5F5D3 0%, #FFFFFF 100%)","borderWidth":"0","width":"100%","borderStyle":"solid"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussnongchanpinxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="view" v-if="isAuth('discussnongchanpinxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('nongchanpinxinxi','删除评论')" type="primary" @click="deleteHandler(scope.row.id ,scope.row.refid)">
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				this.$http({
					url: "discussnongchanpinxinxi/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussdiscussnongchanpinxinxi',query:{refid:id}});
			},
			backClick(){
				history.back()
			},
			discussistopChange(e,row){
				this.$http({
					url:'discussnongchanpinxinxi/update',
					method:'post',
					data:row
				}).then(res=>{})
			},
			...
			async deleteHandler(id ,refid ) {
				...
				await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
					...
					await this.$http({
						url: "discussnongchanpinxinxi/delete",
						method: "post",
						data: ids
					}).then(async ({ data }) => {
						if (data && data.code === 0) {
							this.$http({
								url: 'nongchanpinxinxi/info/' + refid,
								method: "get",
							}).then(res=>{
