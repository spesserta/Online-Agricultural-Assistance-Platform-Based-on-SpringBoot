...
<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"10px 20px","background":" linear-gradient( 180deg, #EFFFEF 0%, #F8F8F8 100%)"}'>
			...
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 30px 0","borderRadius":"16px","background":" linear-gradient( 180deg, #D5F5D3 0%, #FFFFFF 100%)"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","padding":"30px","position":"relative","flexWrap":"wrap","display":"flex"}' >
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"400","height":"40px"}' class="item-label">原料名称</label>
						<el-input v-model="searchForm.yuanliaomingcheng" placeholder="原料名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}' class="select" label="原料分类" prop="yuanliaofenlei">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"400","height":"40px"}' class="item-label">原料分类</label>
						<el-select clearable v-model="searchForm.yuanliaofenlei" placeholder="请选择原料分类" >
							<el-option v-for="(item,index) in yuanliaofenleiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 1% 10px 0","display":"flex"}' class="select">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"400","height":"40px"}' class="item-label">是否通过</label>
						<el-select clearable v-model="searchForm.sfsh" placeholder="是否通过">
							<el-option v-for="(item,index) in sfshOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 1% 10px 0","display":"flex"}' class="select">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"400","height":"40px"}' class="item-label">是否支付</label>
				...
				<el-row class="actions" :style='{"width":"100%","padding":"0 30px 30px 30px","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yuanliaocaigou','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('yuanliaocaigou','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yuanliaocaigou','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe9" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('yuanliaocaigou','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-zhifudingjin" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						批量支付
					</el-button>
					<el-button class="btn18" v-if="isAuth('yuanliaocaigou','采购支出')" type="success" @click="chartDialogShow1">
			...
			<div :style='{"padding":"20px","borderColor":"#212D3F","borderRadius":"16px","background":"linear-gradient( 180deg, #D5F5D3 0%, #FFFFFF 100%)","borderWidth":"0","width":"100%","borderStyle":"solid"}'>
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true'
						...
						label="原料分类">
						<template slot-scope="scope">
							{{scope.row.yuanliaofenlei}}
						</template>
					</el-table-column>
					<el-table-column  :resizable='true' prop="yuanliaozhaopian" width="200" label="原料照片">
						<template slot-scope="scope">
							<div v-if="scope.row.yuanliaozhaopian">
								<img v-if="scope.row.yuanliaozhaopian.substring(0,4)=='http'&&scope.row.yuanliaozhaopian.split(',w').length>1" :src="scope.row.yuanliaozhaopian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.yuanliaozhaopian)">
								<img v-else-if="scope.row.yuanliaozhaopian.substring(0,4)=='http'" :src="scope.row.yuanliaozhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.yuanliaozhaopian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.yuanliaozhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.yuanliaozhaopian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'
												prop="youxiaochengfen"
						label="有效成分">
						<template slot-scope="scope">
							{{scope.row.youxiaochengfen}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'
												prop="jiage"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yuanliaocaigou','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuanliaocaigou','采购配送')" @click="caigoupeisongCrossAddOrUpdateHandler(scope.row,'cross','是','是','[1]','已配送')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								采购配送
							</el-button>
							<el-button class="edit" v-if=" isAuth('yuanliaocaigou','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('yuanliaocaigou','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<caigoupeisong-cross-add-or-update v-if="caigoupeisongCrossAddOrUpdateFlag" :parent="this" ref="caigoupeisongCrossaddOrUpdate"></caigoupeisong-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="caigouzongjiaChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
...
<script>
	...
	import caigoupeisongCrossAddOrUpdate from "../caigoupeisong/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				caigoupeisongCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			caigoupeisongCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.caigoupeisongCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuanliaocaigou');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.caigoupeisongCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.caigoupeisongCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				if(row.sfsh!='是'){
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var caigouzongjiaChart1 = echarts.init(document.getElementById("caigouzongjiaChart1"),'macarons');
					let params = {}
					...
					this.$http({
						url: "yuanliaocaigou/value/yuanliaomingcheng/caigouzongjia",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].yuanliaomingcheng);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].yuanliaomingcheng
								})
							}
							var option = {};
							...
							// 使用刚指定的配置项和数据显示图表。
							caigouzongjiaChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								caigouzongjiaChart1.resize();
							};
						}
					})
				})
			...
			getDataList() {
				...
				if(this.searchForm.yuanliaofenlei!='' && this.searchForm.yuanliaofenlei!=undefined){
					params['yuanliaofenlei'] = this.searchForm.yuanliaofenlei
				}
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "yuanliaocaigou/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
