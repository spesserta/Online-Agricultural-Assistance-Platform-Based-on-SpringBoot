...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="收货时间" prop="shouhuoshijian">
				<el-date-picker
					:disabled="ro.shouhuoshijian"
			...
			<el-form-item class="add-item" label="收货备注" prop="shouhuobeizhu">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.shouhuobeizhu"
					placeholder="收货备注"
					v-model="ruleForm.shouhuobeizhu">
					</el-input>
			</el-form-item>
...
<script>
	export default {
		...
		created() {
			...
			this.ruleForm.shouhuoshijian = this.getCurDateTime()
		},
		methods: {
			getMakeZero(s) {
				return s < 10 ? '0' + s : s;
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='shouhuobeizhu'){
							this.ruleForm.shouhuobeizhu = obj[o];
							this.ro.shouhuobeizhu = true;
							continue;
						}
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`caigoushouhuo/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
					...
					if(valid) {
						if(this.type=='cross'){
							...
							if(crossrefid && crossuserid) {
								...
								await this.$http.get('caigoushouhuo/list', {
									params: params
								}).then(res => {
						...
						await this.$http.post(`caigoushouhuo/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
