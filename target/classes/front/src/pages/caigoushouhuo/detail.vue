	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'-'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/caigoushouhuo?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">收货时间</div>
						<div class="text "  >{{detail.shouhuoshijian}}</div>
					</div>
					<div class="item">
						<div class="lable">收货备注</div>
						<div class="text "  >{{detail.shouhuobeizhu}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('caigoushouhuo','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('caigoushouhuo','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('caigoushouhuo','采购评价')" 
							@click="onAcross('caigoupingjia','','','','')" type="warning">
							采购评价
						</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('caigoushouhuo','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`caigoushouhuo`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/caigoushouhuo', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/caigoushouhuoAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此采购收货？') .then(_ => {
					this.$http.post('caigoushouhuo/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
