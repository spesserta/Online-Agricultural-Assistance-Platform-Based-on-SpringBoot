...
<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="200px"
			>
			<el-form-item class="add-item" label="原料名称" prop="yuanliaomingcheng">
			...
			<el-form-item class="add-item"  label="原料分类" prop="yuanliaofenlei">
				<el-select v-model="ruleForm.yuanliaofenlei" placeholder="请选择原料分类" :disabled="ro.yuanliaofenlei"  filterable>
					<el-option
			...
			<el-form-item class="add-item" label="原料照片" v-if="type!='cross' || (type=='cross' && !ro.yuanliaozhaopian)" prop="yuanliaozhaopian">
				<file-upload
					tip="点击上传原料照片"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="原料照片" prop="yuanliaozhaopian">
				<img v-if="ruleForm.yuanliaozhaopian.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.yuanliaozhaopian.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.yuanliaozhaopian.split(',')" :src="baseUrl+item">
			</el-form-item>
			<el-form-item class="add-item" label="有效成分" prop="youxiaochengfen">
				<el-input v-model="ruleForm.youxiaochengfen" 
					placeholder="有效成分" clearable :readonly="ro.youxiaochengfen"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="采购数量" prop="kucun">
				<el-input v-model.number="ruleForm.kucun" 
					placeholder="采购数量" clearable :readonly="ro.kucun"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="采购总价" prop="caigouzongjia">
				<el-input-number v-model="ruleForm.caigouzongjia" placeholder="采购总价" :disabled="ro.caigouzongjia"></el-input-number>
			</el-form-item>
			...
			<el-form-item class="add-item"  label="配送状态" prop="peisongzhuangtai">
				<el-select v-model="ruleForm.peisongzhuangtai" placeholder="请选择配送状态" :disabled="ro.peisongzhuangtai"  filterable>
					<el-option
			...
			<el-form-item class="add-item" label="配送备注" prop="peisongbeizhu">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.peisongbeizhu"
					placeholder="配送备注"
					v-model="ruleForm.peisongbeizhu">
					</el-input>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='yuanliaozhaopian'){
							this.ruleForm.yuanliaozhaopian = obj[o]?obj[o].split(",")[0]:'';
						...
						if(o=='peisongbeizhu'){
							this.ruleForm.peisongbeizhu = obj[o];
							this.ro.peisongbeizhu = true;
							continue;
						}
					...
					this.ruleForm.peisongzhuangtai = '配送中'; 				}else if(type=='edit'){
					this.info()
				}
				// 获取用户信息
				...
				this.$http.get('option/yuanliaofenlei/yuanliaofenlei', {}).then(res => {
					if (res.data.code == 0) {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`caigoupeisong/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
					...
					if(valid) {
						if(this.type=='cross'){
							...
							if(crossrefid && crossuserid) {
								...
								await this.$http.get('caigoupeisong/list', {
									params: params
								}).then(res => {
						...
						await this.$http.post(`caigoupeisong/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
			...
			yuanliaozhaopianUploadChange(fileUrls) {
				this.ruleForm.yuanliaozhaopian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
