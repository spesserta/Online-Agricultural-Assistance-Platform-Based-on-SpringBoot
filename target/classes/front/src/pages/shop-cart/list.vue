<template>
	<div :style='{"width":"100%","padding":"20px 10% 40px","margin":"10px auto","position":"relative","background":"none"}'>
		<div class="section-title" :style='{"padding":"0","borderColor":"#e61f4d","margin":"10px auto","color":"#000000","textAlign":"center","borderWidth":"0 0 0px","display":"none","width":"100%","lineHeight":"32px","fontSize":"30px","borderStyle":"solid","fontWeight":"600"}'>购物车</div>
		...
		<el-table @selection-change="handleSelectionChange" :data="tableData" style="width: 100%">
			...
			<el-table-column label="价格">
				<template slot-scope="scope">
					<span :style='{"fontSize":"12px"}'>￥</span>{{ (scope.row.price).toFixed(2) }}
				</template>
			</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getCartList() {
				this.$http.get('cart/list', {params: {page: 1, limit: 1000,sort: 'addtime',order:'desc', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
					if (res.data.code == 0) {
