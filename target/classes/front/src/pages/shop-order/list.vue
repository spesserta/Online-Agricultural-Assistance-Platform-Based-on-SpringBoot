<template>
	<div :style='{"width":"100%","padding":"20px 10% 40px","margin":"10px auto","position":"relative","background":"none"}'>
		...
		<div class="section-title" :style='{"padding":"0","borderColor":"#e61f4d","margin":"10px auto","color":"#000000","textAlign":"center","borderWidth":"0 0 0px","display":"none","width":"100%","lineHeight":"32px","fontSize":"30px","borderStyle":"solid","fontWeight":"600"}'>我的订单</div>
		...
		<el-table :data="tableData" style="width: 100%" >
			...
			<el-table-column label="操作" width="150">
				<template slot-scope="scope">
					...
					<el-button v-show="activeName == '已完成'&&scope.row.sfsh==''" type="primary" :style='{"margin":"2px auto",}' size="mini" @click="toDetail(scope.row)">评价</el-button>
							...
							if(item.type==1||item.type==3) {
								...
								this.$http.post(table+'/update', this.user).then(res => {
									...
									this.$http.post(`orders/update`, item).then(res => {
										if(item.tablename == `nongchanpinxinxi`){
											// 加回库存数量
											good.alllimittimes = good.alllimittimes + item.buynumber;
											this.$http.post(`nongchanpinxinxi/update`, good).then(res => {
											});
										}
										this.$message({
