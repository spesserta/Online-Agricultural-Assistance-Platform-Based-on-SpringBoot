...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="高校地址" prop="gaoxiaodizhi">
				<el-input v-model="ruleForm.gaoxiaodizhi" 
					placeholder="高校地址" clearable :readonly="ro.gaoxiaodizhi"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="高校电话" prop="gaoxiaodianhua">
				<el-input v-model="ruleForm.gaoxiaodianhua" 
					placeholder="高校电话" clearable :readonly="ro.gaoxiaodianhua"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="高校封面" v-if="type!='cross' || (type=='cross' && !ro.touxiang)" prop="touxiang">
				<file-upload
					tip="点击上传高校封面"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="高校封面" prop="touxiang">
			...
			<el-form-item class="add-item" label="高校简介" prop="gaoxiaojianjie">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.gaoxiaojianjie"
					placeholder="高校简介"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`gaoxiao/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`gaoxiao/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
