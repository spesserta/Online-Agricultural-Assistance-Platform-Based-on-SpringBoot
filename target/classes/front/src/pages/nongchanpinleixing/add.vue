		...
		<el-form
			...
			<el-form-item class="add-item" label="农产品类型" prop="nongchanpinleixing">
				<el-input v-model="ruleForm.nongchanpinleixing" 
					placeholder="农产品类型" clearable :readonly="ro.nongchanpinleixing"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`nongchanpinleixing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`nongchanpinleixing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
