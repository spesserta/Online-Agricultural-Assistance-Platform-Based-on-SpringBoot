		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="知识分类" prop="zhishifenlei" >
					<el-select :disabled="ro.zhishifenlei" v-model="ruleForm.zhishifenlei" placeholder="请选择知识分类"  filterable>
						<el-option
				...
				<el-form-item class="input" v-if="type!='info'"  label="专业领域" prop="zhuanyelingyu" >
					<el-input v-model="ruleForm.zhuanyelingyu" placeholder="专业领域" clearable  :readonly="ro.zhuanyelingyu"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="专业领域" prop="zhuanyelingyu" >
					<el-input v-model="ruleForm.zhuanyelingyu" placeholder="专业领域" readonly></el-input>
				</el-form-item>
				<el-form-item class="upload" v-if="type!='info' && !ro.zhishizhaopian" label="知识照片" prop="zhishizhaopian" >
					<file-upload
						tip="点击上传知识照片"
						action="file/upload"
						:limit="3"
				...
				<el-form-item class="upload" v-else-if="ruleForm.zhishizhaopian" label="知识照片" prop="zhishizhaopian" >
					<img v-if="ruleForm.zhishizhaopian.substring(0,4)=='http'&&ruleForm.zhishizhaopian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.zhishizhaopian" width="100" height="100" @click="imgPreView(ruleForm.tupian)">
					<img v-else-if="ruleForm.zhishizhaopian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.zhishizhaopian.split(',')[0]" width="100" height="100" @click="imgPreView(ruleForm.tupian.split(',')[0])">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.zhishizhaopian.split(',')" :src="$base.url+item" width="100" height="100" @click="imgPreView($base.url+item)">
				</el-form-item>
				<el-form-item class="fileupload" v-if="type!='info'&& !ro.zhishifujian" label="知识附件" prop="zhishifujian" >
					<file-upload
						tip="点击上传知识附件"
						action="file/upload"
						:limit="1"
				...
				<el-form-item v-else-if="ruleForm.zhishifujian" label="知识附件" prop="zhishifujian" >
					<el-button class="downBtn" type="text" size="small" @click="download($base.url+ruleForm.zhishifujian)">
				...
				<el-form-item class="fileupload" v-if="type!='info'&& !ro.zhishishipin" label="知识视频" prop="zhishishipin" >
					<file-upload
						tip="点击上传知识视频"
						action="file/upload"
						:limit="1"
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="知识内容" prop="zhishineirong" >
				<editor 
			...
			<el-form-item v-else-if="ruleForm.zhishineirong" label="知识内容" prop="zhishineirong"  class="textBox">
...
<script>
	...
	export default {
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='zhishizhaopian'){
							this.ruleForm.zhishizhaopian = obj[o];
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.zhishineirong = this.ruleForm.zhishineirong.replace(reg,'../../../zhunong79djy629/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			// 提交
			async onSubmit() {
					if(this.ruleForm.zhishizhaopian!=null) {
						this.ruleForm.zhishizhaopian = this.ruleForm.zhishizhaopian.replace(new RegExp(this.$base.url,"g"),"");
					}
					if(this.ruleForm.zhishifujian!=null) {
						this.ruleForm.zhishifujian = this.ruleForm.zhishifujian.replace(new RegExp(this.$base.url,"g"),"");
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jishuzhishiku/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jishuzhishikuCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
			...
			zhishizhaopianUploadChange(fileUrls) {
				this.ruleForm.zhishizhaopian = fileUrls;
			},
			zhishifujianUploadChange(fileUrls) {
				this.ruleForm.zhishifujian = fileUrls;
			},
