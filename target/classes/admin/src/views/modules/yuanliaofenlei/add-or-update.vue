...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="原料分类" prop="yuanliaofenlei" >
					<el-input v-model="ruleForm.yuanliaofenlei" placeholder="原料分类" clearable  :readonly="ro.yuanliaofenlei"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="原料分类" prop="yuanliaofenlei" >
					<el-input v-model="ruleForm.yuanliaofenlei" placeholder="原料分类" readonly></el-input>
				</el-form-item>
			</template>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yuanliaofenlei/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yuanliaofenleiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
