	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item v-else class="input" label="库存" prop="kucun" >
			...
			<el-form-item class="textarea" v-if="type!='info'" label="使用说明" prop="shiyongshuoming" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.shiyongshuoming" label="使用说明" prop="shiyongshuoming"  class="textBox">
				<span class="text">{{ruleForm.shiyongshuoming}}</span>
			</el-form-item>
			<el-form-item v-if="type!='info'" class="editorBox" label="原料介绍" prop="yuanliaojieshao" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
			...
			<el-form-item v-else-if="ruleForm.yuanliaojieshao" label="原料介绍" prop="yuanliaojieshao"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.yuanliaojieshao"></span>
			</el-form-item>
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.yuanliaojieshao = this.ruleForm.yuanliaojieshao.replace(reg,'../../../zhunong79djy629/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `nongyeyuanliao/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.nongyeyuanliaoCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
