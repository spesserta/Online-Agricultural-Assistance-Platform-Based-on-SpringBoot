		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 30px 0","borderRadius":"16px","background":" linear-gradient( 180deg, #D5F5D3 0%, #FFFFFF 100%)"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"100%","padding":"0 30px 30px 30px","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('discussjishuzhishiku','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('discussjishuzhishiku','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"20px","borderColor":"#212D3F","borderRadius":"16px","background":"linear-gradient( 180deg, #D5F5D3 0%, #FFFFFF 100%)","borderWidth":"0","width":"100%","borderStyle":"solid"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussjishuzhishiku','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="view" v-if="isAuth('discussjishuzhishiku','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('jishuzhishiku','删除评论')" type="primary" @click="deleteHandler(scope.row.id ,scope.row.refid)">
	...
	export default {
		...
		methods: {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussdiscussjishuzhishiku',query:{refid:id}});
			},
			backClick(){
				history.back()
			},
			discussistopChange(e,row){
				this.$http({
					url:'discussjishuzhishiku/update',
					method:'post',
					data:row
				}).then(res=>{})
			},
			...
			async deleteHandler(id ,refid ) {
				...
				await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
					...
					await this.$http({
						url: "discussjishuzhishiku/delete",
						method: "post",
						data: ids
					}).then(async ({ data }) => {
						if (data && data.code === 0) {
							this.$http({
								url: 'jishuzhishiku/info/' + refid,
								method: "get",
							}).then(res=>{
								if(res.data&&res.data.code==0){
									res.data.data.discussnum = res.data.data.discussnum - Number(ids.length)
									this.$http({
										url: 'jishuzhishiku/update',
										method: "post",
										data:res.data.data
									}).then(res1=>{
