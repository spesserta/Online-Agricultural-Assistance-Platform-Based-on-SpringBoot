		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="配送状态" prop="peisongzhuangtai" >
					<el-select :disabled="ro.peisongzhuangtai" v-model="ruleForm.peisongzhuangtai" placeholder="请选择配送状态"  filterable>
						<el-option
			...
			<el-form-item class="textarea" v-if="type!='info'" label="配送备注" prop="peisongbeizhu" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.peisongbeizhu" label="配送备注" prop="peisongbeizhu"  class="textBox">
				<span class="text">{{ruleForm.peisongbeizhu}}</span>
			</el-form-item>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `caigoupeisong/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.caigoupeisongCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
