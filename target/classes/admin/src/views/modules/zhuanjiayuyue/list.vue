		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 30px 0","borderRadius":"16px","background":" linear-gradient( 180deg, #D5F5D3 0%, #FFFFFF 100%)"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","padding":"30px","position":"relative","flexWrap":"wrap","display":"flex"}' >
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"400","height":"40px"}' class="item-label">高校名称</label>
						<el-input v-model="searchForm.gaoxiaomingcheng" placeholder="高校名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"400","height":"40px"}' class="item-label">专家名称</label>
						<el-input v-model="searchForm.zhuanjiamingcheng" placeholder="专家名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"width":"100%","padding":"0 30px 30px 30px","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('zhuanjiayuyue','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('zhuanjiayuyue','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('zhuanjiayuyue','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"padding":"20px","borderColor":"#212D3F","borderRadius":"16px","background":"linear-gradient( 180deg, #D5F5D3 0%, #FFFFFF 100%)","borderWidth":"0","width":"100%","borderStyle":"solid"}'>
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true'
												prop="zhuanjiamingcheng"
						label="专家名称">
						<template slot-scope="scope">
							{{scope.row.zhuanjiamingcheng}}
						</template>
					</el-table-column>
					<el-table-column  :resizable='true' prop="zhuanjiazhaopian" width="200" label="专家照片">
						<template slot-scope="scope">
							<div v-if="scope.row.zhuanjiazhaopian">
								<img v-if="scope.row.zhuanjiazhaopian.substring(0,4)=='http'&&scope.row.zhuanjiazhaopian.split(',w').length>1" :src="scope.row.zhuanjiazhaopian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.zhuanjiazhaopian)">
								<img v-else-if="scope.row.zhuanjiazhaopian.substring(0,4)=='http'" :src="scope.row.zhuanjiazhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.zhuanjiazhaopian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.zhuanjiazhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.zhuanjiazhaopian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('zhuanjiayuyue','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('zhuanjiayuyue','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('zhuanjiayuyue','删除')" type="primary" @click="deleteHandler(scope.row.id)">
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				if(this.searchForm.gaoxiaomingcheng!='' && this.searchForm.gaoxiaomingcheng!=undefined){
					params['gaoxiaomingcheng'] = '%' + this.searchForm.gaoxiaomingcheng + '%'
				}
				if(this.searchForm.zhuanjiamingcheng!='' && this.searchForm.zhuanjiamingcheng!=undefined){
					params['zhuanjiamingcheng'] = '%' + this.searchForm.zhuanjiamingcheng + '%'
				}
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "zhuanjiayuyue/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
