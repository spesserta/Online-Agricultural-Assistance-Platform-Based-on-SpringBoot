	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="农民姓名" prop="nongminxingming" >
					<el-input v-model="ruleForm.nongminxingming" placeholder="农民姓名" clearable  :readonly="ro.nongminxingming"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="农民姓名" prop="nongminxingming" >
					<el-input v-model="ruleForm.nongminxingming" placeholder="农民姓名" readonly></el-input>
				</el-form-item>
	...
	export default {
		data() {
			...
			return {
				...
				rules: {
					nongminzhanghao: [
						{ required: true, message: '农民账号不能为空', trigger: 'blur' },
					],
					mima: [
						{ required: true, message: '密码不能为空', trigger: 'blur' },
					],
					nongminxingming: [
						{ required: true, message: '农民姓名不能为空', trigger: 'blur' },
					],
					xingbie: [
					],
					lianxidianhua: [
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='nongminxingming'){
							this.ruleForm.nongminxingming = obj[o];
							this.ro.nongminxingming = true;
							continue;
						}
						if(o=='xingbie'){
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `nongmin/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.nongminCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
